...
if [ "${CPU}" = "arm1176jzf-s" ]; then
   CXXFLAGS="-g -O2 -mcpu=${CPU} -mfpu=vfp -mfloat-abi=hard"
elif [ "${CPU}" = "cortex-a53" ]; then
   CXXFLAGS="-g -O2 -mcpu=${CPU} -mfpu=neon-fp-armv8 -mfloat-abi=hard"
elif [ "${CPU}" = "cortex-a7" ]; then
   CXXFLAGS="-g -O2 -mcpu=${CPU} -mfpu=neon-vfpv4 -mfloat-abi=hard"

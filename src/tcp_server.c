...
int tcp_server_init_rx_device(dsp_worker *dsp_worker, tcp_server *server, struct tcp_worker *tcp_worker) {
  ...
  if (server->app_config->rx_sdr_type == SDR_TYPE_SDR_SERVER) {
    ...
    if (closest == NULL) {
      ...
      code = sdr_server_client_create(tcp_worker->id, rx, server->app_config->rx_sdr_server_address, server->app_config->rx_sdr_server_port, server->app_config->read_timeout_seconds, server->app_config->buffer_size, &rx_device);
      if (code != 0) {
        free(rx);
        return -RESPONSE_DETAILS_INTERNAL_ERROR;
      }
...
void handle_tx_client(int client_socket, struct message_header *header, tcp_server *server) {
  ...
  *tcp_worker = (struct tcp_worker){0};
  ...
  int code = modem_create(server->app_config, tcp_worker->tx_req, NULL, NULL, NULL, NULL, &tcp_worker->modem);
  if (code != 0) {
...
void handle_rx_client(int client_socket, struct message_header *header, tcp_server *server) {
  ...
  *tcp_worker = (struct tcp_worker){0};
  ...
  code = dsp_worker_create(tcp_worker->id, tcp_worker->client_socket, server->app_config, tcp_worker->rx_req, &dsp_worker);
  if (code != 0) {
    tcp_server_write_response_and_close(client_socket, RESPONSE_STATUS__FAILURE, RESPONSE_DETAILS_INTERNAL_ERROR);
    tcp_worker_destroy(tcp_worker);
    return;

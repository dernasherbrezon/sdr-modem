...
int dsp_worker_create(uint32_t id, int client_socket, struct server_config *server_config, struct RxRequest *req,
    ...
    *result = (struct dsp_worker_t) {0};
    ...
    if (req->doppler != NULL) {
        ...
        code = doppler_create(doppler_settings->latitude / 10E6, doppler_settings->longitude / 10E6, doppler_settings->altitude / 10E3, req->rx_sampling_freq, req->rx_center_freq, 0, start, server_config->buffer_size, tle, &result->dopp);
        if (code != 0) {

...
int create_aligned_taps(const float *original_taps, size_t taps_len, lpf *filter) {
...
void lpf_process_float(const float *input, size_t input_len, float *working_buffer, void **output, size_t *output_len,
                       lpf *filter) {
...
void lpf_process_complex(const float complex *input, size_t input_len, float complex *working_buffer, void **output,
                         size_t *output_len, lpf *filter) {
    memcpy(working_buffer + filter->history_offset, input, input_len * filter->num_bytes);
    size_t working_len = filter->history_offset + input_len;
    size_t i = 0;
    size_t produced = 0;
...
void lpf_process(const void *input, size_t input_len, void **output, size_t *output_len, lpf *filter) {
    if (filter->num_bytes == sizeof(float complex)) {
        lpf_process_complex((float complex *) input, input_len, (float complex *) filter->working_buffer, output,
                            output_len, filter);
    } else if (filter->num_bytes == sizeof(float)) {
        lpf_process_float((float *) input, input_len, (float *) filter->working_buffer, output, output_len, filter);
    }
}

...
static int cli_create_rx_modem(app_config *config, struct cli_t *result) {
  return modem_create(config, &config->rx_req, config->rx_freq_offset_file, &result->rx_modem);
}

static int cli_create_tx_modem(app_config *config, struct cli_t *result) {
  return modem_create(config, &config->tx_req, config->tx_freq_offset_file, &result->tx_modem);
}

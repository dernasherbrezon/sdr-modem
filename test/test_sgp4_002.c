...
void test_stipping_spaces2() {
  ...
  TEST_ASSERT_EQUAL_STRING(result.tle.sat_name, "");
}

void test_stipping_spaces() {
  ...
  int code = Get_Next_Tle_Set(tle_with_spaces, &result.tle);
  TEST_ASSERT_EQUAL_INT(1, code);
  TEST_ASSERT_EQUAL_STRING(result.tle.sat_name, "01234567890123456789123");
}
...
int main(void) {
  ...
  RUN_TEST(test_stipping_spaces);
  RUN_TEST(test_stipping_spaces2);
  return UNITY_END();
}

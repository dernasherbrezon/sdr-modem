...
void test_invalid_subcarrier_frequency_zero() {
  ...
  TEST_ASSERT_NOT_EQUAL_INT(0, psk_pm_modem_create(&settings, 4096, NULL, NULL, &mod));
}
...
void test_round_trip() {
  ...
  int code = psk_pm_modem_create(&settings, (uint32_t) input_len, NULL, NULL, &mod);
  TEST_ASSERT_EQUAL_INT(0, code);
  code = psk_pm_modem_create(&settings, max_buffer_length, NULL, NULL, &demod);
  TEST_ASSERT_EQUAL_INT(0, code);

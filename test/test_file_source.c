...
void test_cs16_success() {
  int code = file_source_create(1, NULL, FILE_FORMAT_CF32, filename, FILE_FORMAT_CS16, 48000, 2000, &device);
  TEST_ASSERT_EQUAL_INT(0, code);
  ...
  code = file_source_create(1, filename, FILE_FORMAT_CS16, NULL, FILE_FORMAT_CF32, 48000, 2000, &device);
  TEST_ASSERT_EQUAL_INT(0, code);
  complex float *output = NULL;
  size_t output_len = 0;
  device->sdr_process_rx(&output, &output_len, device->plugin);
  TEST_ASSERT_EQUAL_INT(buffer_len, output_len);
  for (size_t i = 0; i < buffer_len; i++) {
    TEST_ASSERT_FLOAT_WITHIN(0.0001f, buffer[2 * i], crealf(output[i]));
    TEST_ASSERT_FLOAT_WITHIN(0.0001f, buffer[2 * i + 1], cimagf(output[i]));
  }
}

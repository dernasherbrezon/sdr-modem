...
START_TEST (test_normal) {
    ...
    float *output = NULL;
    size_t output_len = 0;
    quadrature_demod_process(complex_input, 2, &output, &output_len, quad_demod);

    const float expected[] = {0.000000F,-14.935266F};
    assert_float_array(expected, sizeof(expected) / sizeof(float), output, output_len);

    quadrature_demod_process(complex_input + 2, 198, &output, &output_len, quad_demod);

...
RUN apt-get update && apt-get install --no-install-recommends -y build-essential file valgrind cmake libconfig-dev pkg-config libvolk-dev libprotobuf-c-dev libvolk-bin libiio-dev check librtlsdr-dev zlib1g-dev && rm -rf /var/lib/apt/lists/*